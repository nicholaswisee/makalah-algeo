...
QueueSimulator::QueueSimulator(double arrivalRate, double serviceRate, int maxStates,
                               unsigned int seed)
    : lambda(arrivalRate),
      mu(serviceRate),
      currentTime(0.0),
      currentQueueLength(0),
      maxStateTracked(maxStates),
      rng(seed),
      arrivalDist(arrivalRate),
...
void QueueSimulator::printSummary() const {
    ...
    for (int i = 0; i < std::min(10, maxStateTracked + 1); ++i) {
        std::cout << std::setw(5) << i << " | " << std::setw(10) << simProbs[i] << " | "
                  << std::setw(11) << theoProbs[i] << " | " << std::setw(10)
                  << std::abs(simProbs[i] - theoProbs[i]) << std::endl;
    }
}
